...
export default async function Projects() {
  ...
  const sorted = allProjects
    ...
    .sort(
      (a, b) =>
        new Date(b.project_date ?? Number.POSITIVE_INFINITY).getTime() -
        new Date(a.project_date ?? Number.POSITIVE_INFINITY).getTime()
    );
              ...
              <article className="relative w-full h-full p-4 md:p-8">
                <div className="flex items-center justify-between gap-2">
                  <div className="text-xs text-zinc-100">
                    {featured.project_date ? (
                      <time dateTime={new Date(featured.project_date).toISOString()}>
                        {Intl.DateTimeFormat(undefined, {
                          dateStyle: "medium",
                        }).format(new Date(featured.project_date))}
                      </time>
                    ) : (
                      <span>SOON</span>
                    )}
                  ...
                  {/* <span className="flex items-center gap-1 text-xs text-zinc-500">
                    <Eye className="w-4 h-4" />{" "}{Intl.NumberFormat("en-US", { notation: "compact" }).format(0)}
                  </span> */}
                </div>
